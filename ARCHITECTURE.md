...
## 32. Userspace AmneziaWG Fallback Tunnel (v3.1)

Для обхода сигнатурных L7 DPI-блокировок WireGuard без необходимости создания TUN/TAP сетевых интерфейсов или получения прав локального администратора Windows, в архитектуру был интегрирован легковесный **Userspace AmneziaWG (AWG) туннель**:
- **Чистое Sans-I/O ядро (`wireguard-sans-io`)**: Движок использует I/O-free реализацию протокола WireGuard на Rust. Это исключает системные вызовы, блокирующий ввод-вывод или использование платформозависимых UAPI-сокетов из ядра `boringtun`, делая логику полностью переносимой и асинхронной (поверх Tokio).
- **Маскировка пакетов AmneziaWG (`awg_obfuscator.rs`)**:
  - Выполняет динамическую перезапись 4-байтовых заголовков типов пакетов стандартного WireGuard на обфусцированные magic-значения по умолчанию (`H1 = 512345`, `H2 = 512346`, `H3 = 512347`, `H4 = 512348`) в little-endian байтовом порядке.
  - Накладывает случайный junk-padding на хвосты пакетов рукопожатия и транспорта (`S1 = 120` байт для Initiation, `S2 = 60` байт для Response).
  - Генерирует и отправляет `Jc = 4` мусорных пакета случайной длины (в диапазоне от `Jmin = 40` до `Jmax = 1000` байт) перед отправкой первого пакета рукопожатия, сбивая эвристики DPI-детекторов.
- **Пользовательский UDP-NAT и редирекция (`awg_tunnel.rs`)**:
  - Для направления перехваченного UDP-трафика (такого как DNS-запросы и QUIC/HTTP3) без изменения таблицы маршрутизации ОС, написан легковесный транслятор сетевых адресов на уровне приложения.
  - Карта соединений (`DashMap`) сохраняет соответствие `source_port -> (original_client_ip, source_port)`.
  - Исходящий пакет переписывается на виртуальный адрес интерфейса (по умолчанию `10.0.0.2`), шифруется и уходит на Cloudflare WARP эндпоинт (`engage.cloudflareclient.com:2408`).
  - Входящие ответы деобфусцируются, дешифруются, восстанавливают оригинальные адреса из карты NAT и вбрасываются обратно в стек ОС через raw socket (`inject_raw_udp`).
- **Интеграция с Whitelist Detector**:
  - Перед переключением на AWG движок проверяет доступность UDP-транспорта к Cloudflare.
  - Если UDP режется провайдером полностью (drop-all L3/L4 whitelist), AWG-транспорт отключается, а UDP-пакеты дропаются для принудительного TCP-fallback на Opera HTTPS CONNECT туннели. При критическом падении всех каналов регистрируется ошибка для перевода пользователя на ручную настройку SOCKS5.
- **Автоматическая регистрация устройства (WARP Auto-Registration)**:
  - При отсутствии предустановленных ключей (`private_key` и `public_key`) в конфигурации, движок автоматически инициализирует процесс регистрации нового виртуального устройства в сети Cloudflare WARP.
  - Генерируется случайный локальный Curve25519 ключ, вычисляется соответствующий публичный ключ, и по REST API (`POST https://api.cloudflareclient.com/v0a3121/reg`) запрашивается выделение виртуального IP-адреса и параметров пира.
  - Полученные данные сохраняются в сессионной конфигурации AWG-туннеля и выводятся в логи для возможности их последующего статического сохранения пользователем.
